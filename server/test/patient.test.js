...
const { users, patients, populatePatients } = require("./seed/seed");
  ...
  it("should add new patient", done => {
    chai
      ...
      .end((err, res) => {
        ...
        Patient.findById(res.body._id)
          .then(patient => {
            expect(patient._creator).to.be.deep.equal(users[0]._id);
            expect(patient._id).to.exist;
            expect(patient.name).to.be.equal("Jane");
            expect(patient.lastname).to.be.equal("Taylor");
            expect(patient.age).to.be.equal(13);
            done();
          })
          .catch(err => done(err));
      });
    ...
    chai
      ...
      .end((err, res) => {
        ...
        Patient.findById(hexId).then(patient => {
          ...
          expect(patient.avance).to.exist.and.to.be.equal(
            modifiedPatient.avance
          );
